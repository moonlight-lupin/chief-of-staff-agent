...
Categories appear as user tags (id = display name). Classify / suggest /
prepare still go through the review queue; execute uses guarded
`mail_tag` / `mail_archive` / `mail_create_tag` on the CoS workspace client —
not raw Hermes MCP write tools.
