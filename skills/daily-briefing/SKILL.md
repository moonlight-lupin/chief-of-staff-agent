...
4. **Run Gmail templates.** Execute `briefing_unread_priority`, client-specific `briefing_unread_clients`, and relevant document/invoice templates. Completion criterion: unread count and key threads are deduplicated by thread/message ID. **Reply awareness:** after collecting inbox/unread hits, suppress any message whose thread already contains a later message from the operator (`in:sent` lookback). Do not list a thread as outstanding if the operator has already replied.
...
8. **SA key path mismatch (Phronesis install).** `google_api.py` resolves HERMES_HOME to `~/hermes`, but the key lives in `~/.hermes/secrets/phronesis_service_account.json`. Provider runs (chief_of_staff.py daily) then warn "Not authenticated" for mail/calendar. Fix for the session: `export GOOGLE_SERVICE_ACCOUNT_PATH=/home/phronesis/.hermes/secrets/phronesis_service_account.json` before gmail/calendar calls. Also pass `--account phronesis --as menghuey@phronesis-applied.com` on every call — without the flag the script defaults to the personal OAuth token and exits 'Not authenticated' even with the env var set. Read-only queries only — writes still go through the guarded path. (Provider runs via `workspace_client.get_workspace_client` inject both flags themselves; set the env var only for direct `google_api.py` calls.)
9. **Cron prompt paths may not match the install.** On this install the cron prompt says `~/.hermes/...` but the real tree is `~/hermes/...` (`~/hermes/plugins/chief-of-staff/shared/config/`, `~/hermes/projects/phronesis-applied/`). Resolve every path against the actual filesystem, not the prompt text.
10. **`execute_code` is blocked in cron mode** (no user to approve). For scheduled runs, write a small read-only runner to /tmp that imports `workspace_client.get_workspace_client`, executes the `queries.yaml` templates plus the calendar window, dumps JSON, run it with `terminal(background=true)`, and collect with `process_manage(action='wait')`. No native Gmail/Calendar connectors exist in this session, so ladder path 2 (workspace provider) is the operative read path.
