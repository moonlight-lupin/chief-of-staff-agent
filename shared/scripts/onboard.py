...
def wiki_template_purpose(config: dict[str, Any]) -> str:
    ...
    title = json.dumps(f"{company['name']} Wiki Purpose", ensure_ascii=False)
    return f"""---
title: {title}
type: reference
updated: {generated}
...
def wiki_template_schema(config: dict[str, Any]) -> str:
    ...
    title = json.dumps(f"Wiki Schema — {company['name']}", ensure_ascii=False)
    return f"""---
title: {title}
type: reference
...
def wiki_template_index(config: dict[str, Any]) -> str:
    company = config["company"]
    generated = datetime.now().strftime("%Y-%m-%d")
    return f"""---
title: Wiki Index
...
Seeded by the Chief-of-Staff onboarding wizard for **{company['name']}** on {generated}.
The note-taker skill (`wiki_curator.py refresh`) regenerates this catalog as
pages are added.

## Reference

- [Wiki Purpose](purpose.md) — type: `reference`, mission and operating principles
- [Wiki Schema](SCHEMA.md) — type: `reference`, frontmatter schemas per page type
"""
...
def initialize_wiki(config: dict[str, Any], force: bool = False) -> list[Path]:
    ...
    files = {
        ...
        wiki_path / "index.md": wiki_template_index(config),
    }
    written: list[Path] = []
