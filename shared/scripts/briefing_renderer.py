...
def _weekly_tasks_html(tasks: dict[str, Any]) -> str:
    ...
    return (
        ...
        f'<br>Open overdue (all time): {_esc(tasks.get("tasks_overdue_open", 0))}</p>'
    )
...
def render_weekly_text(briefing: dict[str, Any]) -> str:
    ...
    lines.append(f"- {summary.get('overdue_invoices', 0)} open overdue invoice(s) (all time)")
    ...
    if tasks:
        ...
        lines.append(f"  Overdue open (all time): {tasks.get('tasks_overdue_open', 0)}")
        lines.append("")
...
def render_weekly_html(briefing: dict[str, Any], title: str | None = None) -> str:
    ...
    return (
        ...
        f'<div class="summary">{"".join(_weekly_badges(summary) + _divergence_badges(briefing.get("sources") or {}))}</div>\n'
...
def render_html(briefing: dict[str, Any], title: str | None = None) -> str:
    ...
    trends_html = render_trends_html(trends) if isinstance(trends, dict) else ""
    if trends_html:
        sections_html.append(_html_section(
            "Trends", trends_html, open_by_default=False,
        ))
    style = _HTML_STYLE + (_TREND_STYLE if trends_html else "")
